...
async function resolveTab(info) {
  ...
  if (!tab) {
    tab = await chrome.tabs.create({ url: info.url, active: true });
    info.tabId = tab.id;
  }
  return tab;
}
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  if (info.checksDone >= info.maxChecks) {
    ...
    chrome.action.setBadgeBackgroundColor({ color: "#d40000" }).catch(() => {});
    await stopMonitoring(alarm.name);
    return;
  }
  ...
  await chrome.storage.local.set({ [alarm.name]: info });
});
